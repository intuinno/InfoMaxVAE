...
parser.add_argument(
    ...
    type=str,
    default="mmd",
    choices=["vae", "infomax", "mmd"],
    help="choose a method among [mmd, infomax, vae]",
)
parser.add_argument(
    "--model",
    type=str,
    default="cnn",
    choices=["mlp", "cnn"],
    help="type of encoder and decoder",
)
...
parser.add_argument(
    ...
    type=int,
    default=10,
    help="dim for latent codes [typical values: 10 for mnist, 28 for cifar]",
)
...
parser.add_argument(
    "--batch-size-test", type=int, default=500, help="size of batch for validation"
)
parser.add_argument(
    "--gamma", type=float, default=20.0, help="information preference coefficient"
)
parser.add_argument(
    "--alpha", type=float, default=1.0, help="KL divergence coefficient"
)
parser.add_argument(
    "--lr",
    type=float,
    default=0.001,
    help="learning rate for the vae (encoder and decoder)",
)
parser.add_argument(
    "--print-freq",
    type=int,
    default=100,
    help="frequency to show the progress based on batch index",
)
parser.add_argument(
    "--beta1", type=float, default=0.9, help="beta 1 for adam optimizer"
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=float,
    default=0.00001,
    help="learning for mutual information estimator network",
)
parser.add_argument(
    "--dataset",
    type=str,
...
parser.add_argument(
    "--data-dir", type=str, default="./data", help="directory of dataset (or to save)"
)
...
parser.add_argument(
    ...
    type=int,
    default=5,
    help="frequency to save the progress based on batch index",
)
parser.add_argument(
    "--start-epoch",
    default=0,
...
parser.add_argument(
    "--trial", default=1, type=int, help="auxiliary string to distinguish trials"
)
parser.add_argument(
    "--device", default="cpu", type=str, help="device to use for training (cpu or cuda)"
)
...
def main(args):
    ...
    print(
        f"dataset loaded: {args.dataset} \n number of classes: {classes} \n classes: {num_classes}"
    )

    # data loders
    train_loader = DataLoader(training_set, batch_size=args.batch_size, shuffle=True)
    test_loader = DataLoader(
        test_set, batch_size=args.batch_size_test, shuffle=False, num_workers=3
    )
    ...
    print(
        "total number of trainable parameres in model: {}".format(total_trainable_param)
    )
    ...
    if args.method == "infomax":
        ...
        print(
            "total number of parameters in discriminator: {}".format(total_param_disc)
        )
        ...
        optim_disc = optim.Adam(
            disc.parameters(), lr=args.lr_dis, betas=(args.beta1, args.beta2)
        )
    ...
    for epoch in range(args.start_epoch, args.num_iters):
        ...
        if args.method == "infomax":
            train.train_infomax(
                train_loader, vae, disc, optim_vae, optim_disc, epoch, args
            )
